...
\begin{figure}[h]
  ...
  \begin{subfigure}{0.4\textwidth}
    ...
    \caption{Diagram \cite{_CellDesignerProcessDiagram_} as created with
      \toolname{CellDesigner}. The artifact is a formal description of
...
and exploiting these annotations for classification was problematic (see \refsec{experiments-results}),
...
% the validation set.
% TODO do we need to show that? where do we ref to that?
...
rate (see \reffig{svm-repro-lowlr-reconmapolder-loss}).
...
considering a slightly different setup (see \refsec{importance-reorganisation-steps}).
...
% TODO mention that here then SVM achieves eval performance of nielsen
% TODO elaborate on why that is the case
% ,
% potentially due to fewer contradictory examples. The GNN models show similar
% performance.

% TODO also look at loss here?
...
experiments (see \refsec{hyperparameter-search}). We aim to assess the effect of
...
explore the usefulness of another flavour of message passing, namely \ild{Graph
  Attention} as defined in Equation~\ref{eq:graph-attention}.
...
% TODO chapter intro
% TODO split this from results and move it to methods
...
\reffig{neighb-clust-examples}. All nodes considered have positive
