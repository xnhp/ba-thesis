...
\ild{neuron}, which is defined as the composition of a linear transformation with
...
\vec w + \vec b)$ where $\vec w$ are called the \ild{weights} of the linear
...
% TODO fix: no space after \wrt, replace all occurences
...
One popular choice is the \ild{radial basis function} (RBF) kernel
% \footnote{This
...
% !! TODO also mention \cite{ruiz_identification_2021}
...
features. An overview is given in \reffig{diag-pipeline}.
...
In fact, \toolname{CellDesigner} provides an even more fine-grained
...
direction. An example is given in \reffig{graph-construction}. This yields a
...
An overview over the datasets used in this work is given in \refsec{datasets-used}.
...
be duplicated. We refer to these as \ild{duplication parents}.
...
$\neighb_k^+(v)$ be given as $\{w ~|~ (v,w) \in E(G_{k})\}$ an the \ild{negative
...
numerical vector.
%
In the following, we define several \ild{features}, \ie characteristics of node
...
computed, their resulting vectors are concatenated into a single \ild{feature
...
\ild{embedding} vector by for each GO term that reflects its position in the GO
...
% TODO some blah on ROC etc. (move motivation down from Background)
...
% TODO describe implementation challenges
% -- several different graph interpretations
% -- different data structures for graphs
...
results. A general discussion follows in \refsec{discussion}. The applied
methods and features are described in detail in \refsec{methods}.
...
% TODO DAVID: regularisation? smaller model? low data?
...
\footnote{
  We will consider a different feature set in \refsec{feature-selection}.
},
...
The results are illustrated in \reffig{results-attention}. In comparison to
\reffig{importance-message-passing} and
\reffig{importance-reorganisation-steps}, we seem to not gain any advantage.
...
Results are visualised in \reffig{annotations-results}. Note that since we only
