...
%
\ild{Disease Maps} are large-scale digital diagrams that describe all processes
...
A possible device for improving a preliminary layout is \ild{duplicating} an
% TODO 'device' probably not good term here
...
\ild{binary node classification}, i.e. for making a prediction for previously unseen
...
\textit{biological process} subtree of the GO graph (see \refsec{background}).
%
We represent a GO term by an \ild{embedding vector} representing its position in
...
conclude with a high-level discussion of results (\refsec{discussion}) and
...
% TODO check if footnote is needed
...
% TODO formatting of footnotes (numbers, not unicode symbols pls)
...
Coming with such a representation is called \ild{feature engineering}.
...
Experiments and results are given in \refsec{experiments-results}.
