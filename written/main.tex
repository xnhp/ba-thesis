...
% TODO this is more like motivation for our approach, not Background
...
% TODO should go in introduction
...
% TODO should go to motivation (introduction)
...
are linked by lines representing chemical processes. An example is given in
\reffig{process-diagram-old-vs-new}. Such diagrams have traditionally been
...
$\tau$, yielding the \ild{Receiver Operating Characteristic} (ROC curve).
...
can compute the \ild{Area Under Curve} (AUC) score given as
\begin{align*}
...
% TODO
% such as force-directed approaches
...
% TODO somewhere here, mention how we determine where to stop when training GNN model
...
graph as described in \refsec{graph-interpretation}. Further variants and
...
% DONE Comparison ROC curves between SVM, GNN (compare-experiments)
% DONE cutoffs for SVM, GNN
\begin{figure}[h]
  \centering
  \begin{subfigure}[h]{0.49\linewidth}
...
% DONE loss curves for GNN (compare-models)
\begin{figure}[h]
  \centering
  \begin{subfigure}[h]{0.49\linewidth}
...
\begin{figure}[h]
  ...
  \end{subfigure}
  \caption{(\ADLast $\rightarrow$ \PDMap) \cd{config-gnn-bipartite} is the same
    model as considered in previous experiments. \cd{config-gnn-none} has
...
\footnote{
  We will consider a different featureset later.
},
...
% definition of train/test tasks, internal split, more data
