...
metabolism (see \reffig{disease-map-example}) and a number of smaller pathway graphs obtained from Reactome
...
Additionally, in \toolname{VANTED}, external data can be mapped to network nodes.
...
likely not the case. The species \cd{Ca2+}, for instance, is not duplicated
...
\item Since in the end we aim to assess the heterogeneity of a node's
  ...
  the message-passing step of a neural network (see \refsec{future-work}). Further,
...
We use version \cd{2.01} of 2015-11-20.
...
\begin{figure}[h]
  ...
  \caption[ An overview of characteristics of networks used for training. ]{ An
    ...
    determined as described in \refsec{datasets}. Note that the $y$-axis of the
...
of \nielsen{} (see \cd{basic-both} in \reftab{feature-importance-features}). We
...
GCN layer defined in \refsec{neural-networks} and the hyperparameters identified
in \refsec{hyperparameter-search}. The GNN model operates on
...
the paper, we assume that these rates correspond to a decision threshold  
 ...
 steps differently (see \refsec{datasets}). However, we show in later
...
\reffig{svm-repro-lowlr-reconmapolder-loss}). This means that the model training
...
through reorganisation steps as described in \refsec{datasets}.
%
...
contradictory examples described in \refsec{datasets}. Although we do adress
...
structural features described in \refsec{datasets}, we can only
...
For each of the structural features defined in \refsec{datasets}, \nielsen{}
...
in \refsec{datasets}. For this experiment only, we use a newer version of
\PDMap{} from autumn 2019 because it was easier to extract annotations from it.


% As described in \refsec{feature-engineering}, we consider two approaches 
...
\item As a \cd{baseline} we use the same feature set as used in
  \refsec{importance-reorganisation-steps}, i.e. \cd{basic-both}.
\item To assess the predictive value of the GO-based features in isolation, we consider
  ...
  \refsec{datasets} (the identifier ``constant'' comes from the fact
...
\refsec{datasets}) exceedingly large, we avoid combining them with the
...
edges described in \refsec{edge-attachment} to the \textit{AlzPathway} map. From
...
\reffig{neighb-clust-examples}. All nodes considered have positive ground-truth
class (i.e. are real duplication parents).

% TODO maybe do comparison to *real* split? (or future work)

% sa40 is representative species alias in collapsed version of ADLast
% want to figure out how it was duplicated
...
% in Dicussion, mention that 'degree works well as predictor' sort of
% supports previous work that would decide based on node degree; now supporting
% this from a different angle (inferred from training data)
...
% examine (mis)classifications

% GNNs able to handle nasty case (with reorg seq) better, but svm catches up
% without reorg steps? (without contradictory examples)
...
% future work: looking at examples would be interesting here
% future work: much more simple classifier on node degree, or see if not
...
% ambuigity due to GO annots on species instead of species aliases: simply
% consider stddev of annots on single node?
